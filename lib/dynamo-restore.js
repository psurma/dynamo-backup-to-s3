...
function DynamoRestore(options) {
    ...
    if (options.awsAccessKey && options.awsSecretKey) {
        params.accessKeyId = options.awsAccessKey;
        params.secretAccessKey = options.awsSecretKey;
    }
	this.error = false
...
DynamoRestore.prototype._checkTableExists = function(error, data) {
	if (error || !data || !data.TableNames){
		...
		else this.emit('error', 'Fatal Error. Could not connect to AWS DynamoDB engine. Please check your credentials.');
		return
	}
...
DynamoRestore.prototype._createTable = function(params) {
  params = this._removeUnsupportedTableParams(params)
	var dynamodb = new AWS.DynamoDB();
...
DynamoRestore.prototype._removeUnsupportedTableParams = function(p) {
  // Strips 'p' down to the properties specified in 'Request Syntax' at: http://docs.aws.amazon.com/amazondynamodb/latest/APIReference/API_CreateTable.html

  var deleteAdditionalProps = function(obj, allowedProps) {
    Object.keys(obj).filter(function (prop) {
        return allowedProps.indexOf(prop) === -1
    }).forEach(function (disallowedProp) {
        delete obj[disallowedProp];
    });
  }

  var deleteAdditionalPropsInArray = function(array, allowedProps) {
    array.forEach(function(item) {
      deleteAdditionalProps(item, allowedProps)
    })
  }
  ...
  // Root
  deleteAdditionalProps(clone, ['AttributeDefinitions', 'GlobalSecondaryIndexes', 'KeySchema', 'LocalSecondaryIndexes', 'ProvisionedThroughput', 'StreamSpecification', 'TableName'])

  // AttributeDefinitions
  if (clone.AttributeDefinitions) {
    deleteAdditionalPropsInArray(clone.AttributeDefinitions, ['AttributeName', 'AttributeType'])
  }

  // GlobalSecondaryIndexes
  if (clone.GlobalSecondaryIndexes) {
    clone.GlobalSecondaryIndexes.forEach(function(index) {
      deleteAdditionalProps(index, ['IndexName', 'KeySchema', 'Projection', 'ProvisionedThroughput'])

      if (index.KeySchema)
        deleteAdditionalPropsInArray(index.KeySchema, ['AttributeName', 'KeyType'])

      if (index.Projection)
        deleteAdditionalProps(index.Projection, ['NonKeyAttributes', 'ProjectionType'])

      if (index.ProvisionedThroughput)
        deleteAdditionalProps(index.ProvisionedThroughput, ['ReadCapacityUnits', 'WriteCapacityUnits'])
    })
  }

  // KeySchema
  if (clone.KeySchema)
    deleteAdditionalPropsInArray(clone.KeySchema, ['AttributeName', 'KeyType'])

  // LocalSecondaryIndexes
  if (clone.LocalSecondaryIndexes) {
    clone.LocalSecondaryIndexes.forEach(function(index) {
      deleteAdditionalProps(index, ['IndexName', 'KeySchema', 'Projection'])
  ...
  // ProvisionedThroughput
  if (clone.ProvisionedThroughput)
    deleteAdditionalProps(clone.ProvisionedThroughput, ['ReadCapacityUnits', 'WriteCapacityUnits'])

  // StreamSpecification
  if (clone.StreamSpecification)
    deleteAdditionalProps(clone.StreamSpecification, ['StreamEnabled', 'StreamViewType'])
